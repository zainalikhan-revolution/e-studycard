function TutorCard({ tutor, onSelect }) {
  try {
    return (
      <div className="tutor-card group" data-name="tutor-card" data-file="components/TutorCard.js">
        <div className={`w-full h-48 bg-gradient-to-br ${tutor.color} rounded-lg mb-4 overflow-hidden transform group-hover:scale-105 transition-transform relative`}>
          {tutor.image ? (
        ...
        <div className="mb-2">
          <span className="text-xs font-semibold text-[var(--primary-color)]">{tutor.emoji} {tutor.subject}</span>
        </div>
        
        <h3 className="text-xl font-bold mb-1">{tutor.name}</h3>
        <p className="text-sm text-gray-400 mb-3">{tutor.urduName}</p>
        <p className="text-sm text-gray-500 mb-4">{tutor.description}</p>
        ...
        <div className="flex gap-2">
          ...
          >
            <div className="icon-video text-base"></div>
            Video
          </button>
        </div>
      </div>
